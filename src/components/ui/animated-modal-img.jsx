      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ref={modalRef}
            className={cn(
              "h-full top-10  w-full container mx-auto bg-white dark:bg-neutral-950 border dark:border-neutral-800 rounded-2xl relative z-50 flex flex-col overflow-hidden",
              className
            )}
...
const CloseIcon = () => {
  const { setOpen } = useModal();
  return (
    <button onClick={() => setOpen(false)} className="absolute  top-2 xl:-top-1 xl:right-12 right-4">
      <svg
        ...
        className="text-white dark:text-white h-10  w-10"
      >
